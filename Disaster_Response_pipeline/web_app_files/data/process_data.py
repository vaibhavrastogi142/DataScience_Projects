...
def clean_data(df):
    ...
    for column in new_category:
        ...
        new_category[column] = new_category[column].str.slice(start=-1)
    ...
    df = df.join(new_category)
    # drop duplicates
    df = df.drop_duplicates()
    #drop the column labeled "original" because it is redundant and has null values
    df = df.drop(['original'], axis=1)
    #drop rows with null values - 163 rows
    df = df.dropna(axis=0)
    return df
